...
You can check the Wii's [live(-ish) status page](/status) for system load info.
{{< /box >}}
...
To compensate, I cheated by adding `ntpdate` to the main `crontab`, so the system would run it hourly at `:42` min past the hour. We still get timesync, but we don't need to sacrifice a sixth of our RAM keeping it resident:

```text
42 * * * * root /usr/sbin/ntpdate time.cloudflare.com
```
...
### Addendum
**Update (Feb 2026):** Since posting this article, plenty of people let me know that configuring the Wii to boot directly into NetBSD is as simple as [installing Priiloader](https://wii.hacks.guide/priiloader.html) and [configuring NetBSD as the Homebrew app to autoboot](https://wii.hacks.guide/priiloader-usage#autobooting-a-homebrew-application). I really don't know why it took me so long to actually configure this, but I wish I'd done it sooner as it allows for proper fully-headless operation... without the mission-critical Wiimote.
