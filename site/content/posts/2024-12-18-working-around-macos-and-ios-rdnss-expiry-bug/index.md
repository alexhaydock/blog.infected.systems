...
We can also implement this fix while still staying in spec with [RFC 8106](https://www.rfc-editor.org/rfc/rfc8106), which defines compliant values for the RDNSS lifetime. It dictates that the lifetime SHOULD be set to a value of _at least_ 3 times the value of `MaxRtrAdvInterval` (which is defined in [RFC 4861](https://www.rfc-editor.org/rfc/rfc4861) as being a maximum of 1800 seconds). There's no maximum value defined in the RFC for an RDNSS lifetime. So - for those counting - as long as we set our RDNSS lifetime to more than 5400 seconds, we can guarantee we're within RFC 8106 spec.
