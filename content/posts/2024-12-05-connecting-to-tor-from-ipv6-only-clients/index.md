...
If you still experience issues you can try the following config which is even more explicit about enforcing IPv6:
...
You may also find that you need to stop Tor, clear out the contents of `/var/lib/tor`, and then restart the service before it actually starts working (though I don't find this is always the case). I'm not too sure why, but I expect this is needed when the client has already selected and cached a set of persistent IPv4 entry guards.
